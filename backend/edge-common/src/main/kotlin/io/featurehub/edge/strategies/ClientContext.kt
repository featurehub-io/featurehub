 ...
 */
class ClientContext(val isClientEvaluation: Boolean, val attributes: Map<String, List<String>>) {
  ...
  operator fun get(key: String, defaultValue: String?): List<String>? {
  ...
  companion object {
    ...
    fun decode(headers: Map<String, List<String>>?, apiKeys: List<KeyParts>): ClientContext {
      ...
      return ClientContext(apiKeys.stream().anyMatch { k: KeyParts -> k.serviceKey.contains("*") }, attributes)
    }
    ...
    fun decode(headers: List<String>?, apiKeys: List<KeyParts>): ClientContext {
      ...
      return ClientContext(apiKeys.any { k: KeyParts -> k.serviceKey.contains("*") }, attributes)
    }
  }
}
