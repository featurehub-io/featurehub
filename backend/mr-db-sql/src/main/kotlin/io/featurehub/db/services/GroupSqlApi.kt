...
open class GroupSqlApi @Inject constructor(
  ...
  override fun createGroup(portfolioId: UUID, group: CreateGroup, whoCreated: Person?): Group? {
...
//    opts: Opts
  ...
  override fun getGroup(gid: UUID, opts: Opts, person: Person): Group? {
    return getGroup(gid, opts, person.id!!.id)
  }
  ...
  private fun updateApplicationMembersOfGroup(
    ...
    finder = if (appId == null) {
      finder.application.isNotNull
    } else {
      finder.application.id.eq(appId)
    }
...
//            removedAcls.add(acl)
  ...
  override fun adminGroupsPersonBelongsTo(personId: UUID): List<DbGroup> {
    return QDbGroup().select(
      QDbGroup.Alias.owningPortfolio.id,
  ...
  private fun updateEnvironmentMembersOfGroup(
    ...
    QDbAcl().group
      ...
      .findEach { acl: DbAcl ->
        ...
        if (acl.environment != null) {
          desiredEnvironments[acl.environment.id]?.let { egr ->
    ...
    for (ae in addedEnvironments) {
      desiredEnvironments[ae]?.let { egr ->
        if (egr.roles.isNotEmpty()) {
  ...
  private fun updateMembersOfGroup(gp: Group, group: DbGroup, personModifiyingUserGroup: UUID): SuperuserChanges? {
    ...
    val addedPeople = gp.members.mapNotNull { it?.id?.id }.toMutableSet()
  ...
  override fun findPortfolioOfGroup(groupId: UUID): UUID? {
    val groups = QDbGroup()
      .select(QDbGroup.Alias.owningPortfolio.id)
