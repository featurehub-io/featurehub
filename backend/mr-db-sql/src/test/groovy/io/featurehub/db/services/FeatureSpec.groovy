...
//      updating.add(found.find({it.key == 'FEATURE_FVU_4'}).valueBoolean(true).locked(true))
  ...
  def "as admin i can filter what features I want for an application"() {
    ...
    when: "i request application features for 'desc'"
      ApplicationFeatureValues withDesc = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, 'desc', null, null, null, null, [], holder.featureFilter)
    then:
      withDesc.features.size() == 3
      withDesc.environments[0].features.size() == 3
    when: 'application features with null'
      ApplicationFeatureValues withNull = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, null, null, null, null, null, [], holder.featureFilter)
    then:
      withNull.features.size() == 4
      withNull.environments[0].features.size() == 4
    when: 'with not'
      ApplicationFeatureValues withNot = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, 'not', null, null, null, null, [], holder.featureFilter)
    then:
      withNot.features.size() == 2
      withNot.environments[0].features.size() == 2
    when: 'with description'
      ApplicationFeatureValues withDescription = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, 'description', null, null, null, null, [], holder.featureFilter)
    then:
      withDescription.features.size() == 2
      withDescription.environments[0].features.size() == 2
    when: 'with IRINA'
      ApplicationFeatureValues withIrina = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, 'IRINA', null, null, null, null, [], holder.featureFilter)
    then:
  ...
  def "i should be able to paginate through the results"() {
    ...
    and: "i request only number features"
      def onlyNumber = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, null,
              null, null, [FeatureValueType.NUMBER], null, [], holder.featureFilter)
    and: "i request number and boolean features"
      def numAndBool = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, null,
              null, null, [FeatureValueType.NUMBER, FeatureValueType.BOOLEAN], null, [], holder.featureFilter)
    then:
  ...
  def "as a user who has no access to the portfolio, i cannot see any features"() {
    ...
    and: "i create some features"
      appApi.createApplicationFeature(app2Id, new CreateFeature().description("x").name('FEATURE_IRINA').key('FEATURE_IRINA').description('Some description').valueType(FeatureValueType.BOOLEAN), superPerson, Opts.empty())
    when: 'application features with null'
      ApplicationFeatureValues withNull = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageIrinaNotPortfolioMember, null, null, null, null, null, null, holder.featureFilter)
    then:
      withNull == null
  }
  ...
  def "as a user who has access to a group but not this application, i cannot see any features"() {
    ...
    when: 'application features with null filter'
      ApplicationFeatureValues withNull = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageJoeMemberOfPortfolio1, null, null, null, null, null, null, holder.featureFilter)
    then:
      withNull == null
  }
  ...
  def "as a group user in an application, i can filter for features i want in an application"() {
    ...
    and: "i allow average joe access to access the environment"
      ...
      groupSqlApi.updateGroup(g1.id, g1, null, true, true, true, Opts.empty());
    when: "i request application features for 'desc'"
      ApplicationFeatureValues withDesc = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageJoeMemberOfPortfolio1, 'desc', null, null, null, null, null, holder.featureFilter)
    and: 'application features with null'
      ApplicationFeatureValues withNull = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageJoeMemberOfPortfolio1, null, null, null, null, null, null, holder.featureFilter)
    and: 'with not'
      ApplicationFeatureValues withNot = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageJoeMemberOfPortfolio1, 'not', null, null, null, null, null, holder.featureFilter)
    and: 'with description'
      ApplicationFeatureValues withDescription = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageJoeMemberOfPortfolio1, 'description', null, null, null, null, null, holder.featureFilter)
    and: 'with IRINA'
      ApplicationFeatureValues withIrina = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageJoeMemberOfPortfolio1, 'IRINA', null, null, null, null, null, holder.featureFilter)
    then:
      withDesc.features.size() == 3
      withDesc.environments[0].features.size() == 3
      withNull.features.size() == 4
  ...
  def "i can block update a bunch of feature values for an application"() {
    ...
    and: "i ask for irina's api"
      ApplicationFeatureValues afv = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, superPerson, null, null, null, null, null, null, holder.featureFilter)
      ApplicationFeatureValues afvAverageJoe = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, averageJoeMemberOfPortfolio1, null, null, null, null, null, null, holder.featureFilter)
      ApplicationFeatureValues afvPortfolioAdminOfPortfolio1 = featureSqlApi.findAllFeatureAndFeatureValuesForEnvironmentsByApplication(app2Id, portfolioAdminOfPortfolio1, null, null, null, null, null, null, holder.featureFilter)
    and:
