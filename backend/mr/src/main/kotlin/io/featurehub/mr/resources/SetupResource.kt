...
class SetupResource @Inject constructor(
    ...
    override fun isInstalled(): SetupResponse {
        ...
        if (setupApi.initialized()) {
            ...
            if (authProviderCollection.providers.size == 1 && loginDisabled) { // only 1 external one
    ...
    private fun fillProviderInfo(): Map<String, IdentityProviderInfo> {
        ...
        authProviderCollection.providers
            .forEach(Consumer { am: AuthProviderSource ->
    ...
    override fun setupSiteAdmin(setupSiteAdmin: SetupSiteAdmin): TokenizedPerson {
        ...
        if (setupSiteAdmin.portfolio.trim { it <= ' ' }.length == 0) {
            throw BadRequestException("Portfolio cannot be 0 length")
        }
        if (setupSiteAdmin.organizationName.trim { it <= ' ' }.length == 0) {
    ...
    private fun createPortfolio(setupSiteAdmin: SetupSiteAdmin, organization: Organization, person: Person?) {
        //this should create portfolio
        try {
            val portfolio = portfolioApi.createPortfolio(
                Portfolio().name(setupSiteAdmin.portfolio).organizationId(organization.id),
                Opts.empty(),
                person!!
            )
            if (person != null) {
