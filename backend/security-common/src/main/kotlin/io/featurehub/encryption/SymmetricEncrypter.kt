...
class SymmetricEncrypterImpl : SymmetricEncrypter {
  ...
  override fun encrypt(source: String, password: String, salt: String): String {
    try {
      ...
      // create IV
      val iv = getInitializationVector(ivLengthInBytes)

      // encrypt source using secret key and IV
      val encryptedBytes = encrypt(source, secretKey, iv)

      // base64 encode encrypted text along with IV
      return encodeCipherTextWithIv(encryptedBytes, iv)
    } catch (e: Exception) {
       throw RuntimeException("Error occurred when trying to encrypt source $source", e)
    }
  ...
  override fun decrypt(encryptedSource: String, password: String, salt: String): String {
    ...
    try {
      ...
      val plainTextBytes =
        decrypt(decodedCipher.cipherText, secretKey, decodedCipher.initializationVector)

      // decode to String
      return String(plainTextBytes)
    } catch (e: Exception) {
      throw RuntimeException("Error occurred when trying to decrypt source $encryptedSource", e)
    }
   ...
   */
  internal fun getInitializationVector(size: Int): ByteArray {
  ...
  internal fun getAESKeyFromPassword(password: String, salt: String): SecretKey {
   ...
   */
  private fun encodeCipherTextWithIv(cipherText: ByteArray, initializationVector: ByteArray): String {
   ...
   */
  private fun decodeCipherTextWithIv(encryptedSource: String, ivLengthByte: Int): DecodedCipher {
   ...
   * @return base64 encoded string containing the encrypted text and initialization vector
   */
  private fun encrypt(source: String, secretKey: SecretKey, initializationVector: ByteArray): ByteArray {
   ...
   * @return
   */
  private fun decrypt(encryptedBytes: ByteArray, secretKey: SecretKey, initializationVector: ByteArray): ByteArray {
    val cipher = Cipher.getInstance(cipherTransformation)
