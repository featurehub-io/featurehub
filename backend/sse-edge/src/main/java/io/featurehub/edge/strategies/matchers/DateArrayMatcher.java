      ...
      switch (attr.getConditional()) {
        case EQUALS: // all match makes no sense
          return attr.getValues().stream()
            .anyMatch(v -> suppliedDate.equals(LocalDateTime.from(DateTimeFormatter.ISO_DATE.parse(v.toString()))));
        case ENDS_WITH:
          break;
        case STARTS_WITH:
          break;
        case GREATER:
          return attr.getValues().stream()
            .anyMatch(v -> suppliedDate.compareTo(LocalDateTime.from(DateTimeFormatter.ISO_DATE.parse(v.toString()))) > 0);
        case GREATER_EQUALS:
          return attr.getValues().stream()
            .anyMatch(v -> suppliedDate.compareTo(LocalDateTime.from(DateTimeFormatter.ISO_DATE.parse(v.toString()))) >= 0);
        case LESS:
        ...
        case NOT_EQUALS:
          return attr.getValues().stream()
            .noneMatch(v -> suppliedDate.equals(LocalDateTime.from(DateTimeFormatter.ISO_DATE.parse(v.toString()))));
