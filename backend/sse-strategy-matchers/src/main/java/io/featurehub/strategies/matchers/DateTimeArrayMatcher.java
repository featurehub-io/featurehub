...
public class DateTimeArrayMatcher implements StrategyMatcher {
  ...
  public boolean match(String suppliedValue, RolloutStrategyAttribute attr) {
    ...
    try {
      Supplier<LocalDateTime> suppliedDate = () -> LocalDateTime.from(formatter.parse(suppliedValue));

      switch (attr.getConditional()) {
        case EQUALS: // all match makes no sense
        case INCLUDES: // same as equals
          return attr.getValues().stream()
            .anyMatch(v -> suppliedDate.get().equals(LocalDateTime.from(formatter.parse(v.toString()))));
        case ENDS_WITH:
        ...
        case GREATER:
          return attr.getValues().stream()
            .anyMatch(v -> suppliedDate.get().compareTo(LocalDateTime.from(formatter.parse(v.toString()))) > 0);
        case GREATER_EQUALS:
          return attr.getValues().stream()
            .anyMatch(v -> suppliedDate.get().compareTo(LocalDateTime.from(formatter.parse(v.toString()))) >= 0);
        case LESS:
        ...
        case EXCLUDES: // same as not_equals
          return attr.getValues().stream()
            .noneMatch(v -> suppliedDate.get().equals(LocalDateTime.from(formatter.parse(v.toString()))));
        case REGEX:
