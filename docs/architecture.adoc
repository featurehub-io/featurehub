...
FeatureHub remains a streaming platform, but it is now also available as a non-streaming platform. It is available
as a bundle (for streaming this is the Party Server, for non-streaming its the Party-Server-Ish) or as individual
...
The non-streaming platform is designed to scale to less - tens of thousands, possibly
more if you have a limited number of environments, or a larger number of read replicas.   
But the non-streaming platform is designed to also be much
...
This can go down without affecting the operation of end-user clients, all their data is in the cache (or in
the database if you use party-server-_ish_ or _edge-rest_). It is only if
