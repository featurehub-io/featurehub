...
FeatureHub supports two types of keys: *Client Evaluated API Keys* and *Server Evaluated API Keys.* 

The version 2.0 of
the SDKs introduce a _Context first API_. This means you define information about who is using the features (including anonymous users), and then the SDK will evaluate the feature values based on that information. 
