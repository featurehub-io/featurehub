...
When you create a feature and set initial value, it is treated as a default strategy and a default value (set per each environment). If you decide to split your rollout you can add a rollout strategy and set a feature value for this strategy different from the default.
...
Each rule is essentially a field, a condition (equals, includes, etc) and one or more values. Whereas each targeting rule
is an `AND` condition, each _value_ is an `OR` condition. For example, if the `country` is `New Zealand` `OR` `Indonesia`
`AND` the custom field `payment_method` is equal to `credit_card` *OR* `dircet_debit`.

Each rollout strategy has zero or more rules associated with it. If it has zero rules and no percentage rule 
the strategy will be ignored. There is no limit on how many rules you can apply.
