...
This will kick off the example app that can be accessed on port 5000. Experiment with the sample app - add a few todo's using "lower case".
If you create a feature flag in the FeatureHub Admin Console called `FEATURE_TITLE_TO_UPPERCASE`, unlock it and set it to true. Add another "to-do" and see how items now being served in "upper case". This flag is affecting the backend service as this is where the feature is implemented using Typescript FeatureHub SDK.
