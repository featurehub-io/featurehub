...
NOTE: As FeatureHub is an open source project, we provide test on only a limited number of databases, primarily Postgres.

We provide and maintain database migrations scripts for MySQL/MariaDB, MS SQL Server, but we do not
actively test on them. 
...
much an evaluation style of deployment. Again, please see how installation documentation on the implications
of this setup, and how you can turn it into a production deployment.
