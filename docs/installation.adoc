...
[options="header"]
|===================================
|Streaming|How Much Use|Option|Recommendation
...
as the basic evaluation image with the difference being the database (which we recommend Postgres).
...
You can run up multiple Party Servers, but the `nats.urls` configuration (see below) must
...
See the docker-compose section below for examples on how to configure this server.
...
FeatureHub Server, means you can deploy these components independently for scalability and redundancy, and Docker images
...
=== Docker-Compose / Docker-Stack / Kubernetes
