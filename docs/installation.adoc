...
- `register.url` [[register-url, Register URL]] - the url used for registration. The front-end should strip the prefix off this and add its own relative one. The format has to
...
- `featurehub.url-path` - this allows you to not host the FeatureHub site on / and is useful if you are not allocating a full url to FeatureHub because
of restrictions in your company domain setup. This will set the <base> href in the index.html of the website and the
front-end will also know how to make calls. If you have extracted the front-end and put it on a CDN, you will need to make
that change yourself. Note, this is an offset, not a full domain name, e.g. `featurehub.url-path=/my-group/featurehub`. This will affect your
<<register-url,registration url>> in the Management Repository, so you will need to make sure you change that as well.
