...
be `register.url=https://some.domain.info/register-url?token=%s`. _This is honoured but no longer required and it is recommended you remove it._
...
that change yourself. Note, this is an offset, not a full domain name, e.g. `featurehub.url-path=/my-group/featurehub`.
