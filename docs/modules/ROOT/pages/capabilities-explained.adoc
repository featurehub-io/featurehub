...
Fastly CDN is supported for all available SDKs for SSE and Polling. Please check in the capabilities table above.
...
This is a much more advanced feature that we have introduced in 2026. It tracks the usage and evaluation of each feature
and allows you to pass it off to your tracking system of choice. Two examples are generally given:

- *OpenTelemetry*, so 
it can attach the information about feature evaluation or the current state of features to existing spans using attributes or events
- *Twilio Segment*, so that tracking events can be sent along with attached user information if required to see which features
are being used and integrate that with whatever analytics you are using.
