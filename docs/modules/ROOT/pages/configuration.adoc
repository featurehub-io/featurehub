...
Edge (streaming), Dacha and Dacha2 do not. 
...
NOTE: If you use Dacha1, you do not need to change anything, the preconfigured settings are all normal.

To swap from Dacha1 (NATS only) to Dacha2 (all options), you need to set the environment variables:

- `dacha1.enabled=false` - tell FeatureHub we are no longer using Dacha1
- `dacha2.enabled=true`
...
If you have only once instance of a
...
You will also need to tell FeatureHub what the topics are for publishing on. There is no harm in configuring these
...
This is the support for AWS Kinesis. We are not fully happy with the performance of this, the polling only mode and the delivery jitter 
means it is not passing our strenuous high performance testing harness. If you are evaluating FeatureHub and want to deploy on AWS, please contact us via our Website and consider
working with us. As with PubSub, you must turn nats support off.
...
Then MR, Edge and MR need to know the streams:  
...
Then MR needs to know the stream for TestSDK updates:

- `cloudevents.inbound.stream-names=featurehub-edge-updates` - 

And Edge needs to know the stream to publish them on:
