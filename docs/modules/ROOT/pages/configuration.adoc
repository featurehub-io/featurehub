...
To enable link:slack-integration.adoc[Slack configuration] or URL and header encryption for Webhooks,
...
- `webhooks.encryption.password` - this was set at a default value in `1.8.0` but it was removed as a security precaution in `1.8.1`. If you have installed and used encryption in `1.8.0` we recommend removing any security tokens (e.g. Slack Bot User OAuth Token) from the database and changing your token. This can
