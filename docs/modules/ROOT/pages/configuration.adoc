...
Kafka does not support Dacha 1, but all other streaming versions are supported (Party Server and the Dacha2 oriented separate variants). You need to make sure FeatureHub is not using NATs, its default platform by default.

- `nats.enabled=false`

The following are the configuration settings for Kafka.

==== Enabling Kafka

Kafka is enabled automatically when `cloudevents.kafka.bootstrap.servers` is set. To explicitly disable it even when bootstrap servers are configured, set:

- `cloudevents.kafka.enabled=false`

==== Connection

- `cloudevents.kafka.bootstrap.servers` *(required)* - comma-separated list of Kafka broker addresses, e.g. `broker1:9092,broker2:9092`

==== Security (optional)

- `cloudevents.kafka.security.protocol` - e.g. `SASL_SSL` or `SSL`. Omit for plaintext.
- `cloudevents.kafka.sasl.mechanism` - e.g. `PLAIN` or `SCRAM-SHA-256`
- `cloudevents.kafka.sasl.jaas.config` - the full JAAS config string, e.g. `org.apache.kafka.common.security.plain.PlainLoginModule required username="user" password="pass";`

==== Producer Tuning

- `cloudevents.kafka.producer.acks` (default `1`) - producer acknowledgement level. `0` = no ack, `1` = leader ack, `all` = full ISR ack.
- `cloudevents.kafka.producer.linger.ms` (default `5`) - how long the producer will wait to batch messages before sending.
- `cloudevents.kafka.compress-events` (default `true`) - whether CloudEvents are compressed before publishing. If it is true, the body is a json+gzip format, making it unreadable to casual browsing, but usually a lot smaller. 

==== Consumer Tuning

- `cloudevents.kafka.consumer.poll.timeout.ms` (default `1000`) - how long each consumer poll will wait for records before returning.
- `cloudevents.kafka.consumer.auto-topic-creation` (default `false`) - whether consumers are allowed to auto-create topics. It is recommended to create topics in advance, set their number of partitions and replication factors.

==== Topics

You need to create the following topics in Kafka (or enable auto-creation). The topic names are configurable:

- `cloudevents.inbound.kafka.mr-features-name` (default `featurehub-mr-dacha2`) - MR publishes feature, environment and service-account updates; Dacha2 subscribes.
- `cloudevents.mr-edge.kafka.topic-name` (default `featurehub-mr-edge`) - MR/Dacha2 publishes enriched feature updates; Edge subscribes.
- `cloudevents.edge-mr.kafka.topic-name` (default `featurehub-edge-updates`) - Edge publishes SDK test updates and webhook results back to MR.
- `cloudevents.edge-mr.kafka.group-id` (default `featurehub-mr-update`) - consumer group ID used by MR when reading from the above topic.
- `cloudevents.stats.kafka.topic-name` (default `featurehub-stats`) - Edge publishes usage stats here. Only needed if stats publishing is enabled.

=== AWS Kinesis

We were supporting AWS Kinesis from v1.5.9 of FeatureHub. We discontinued support in 1.8.0 due to lack of interest. If you are interested and want to work with us to ensure compatibility, please let us know.
...
- `dacha2.use-old-cache` - As of 1.10 of FeatureHub, we have swapped from using the Google Guava cache to the much faster, memory efficient Caffeine cache. If you set this value to `true` it will use the Guava cache used in previous versions. We only intend to keep this code around for a single version and then remove it, so if you find yourself
using it, please let us know with an open issue on Github.
