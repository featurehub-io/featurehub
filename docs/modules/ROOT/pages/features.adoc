...
image::fh_restore_feature.png[Restore Feature, 1500]

NOTE: You cannot restore a feature if another feature with the same key already exists in the application (for example, if the key was reused after the original was deleted). Rename or archive the conflicting feature first, then restore.
