...
`metaData` property is only available through the Admin SDK API (ADK). The intention of this field is to store additional information that you may need to provide about your feature, for example when you want to access this information from your CI/CD pipeline. This could be key-value pairs, or any other metadata. It is stored as a string and has no data limit (stored as CLOB).
