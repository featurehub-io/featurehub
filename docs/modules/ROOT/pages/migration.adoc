...
The requirements for supporting an eventing system are well defined and Cloud Events are used everywhere, so adding new functionality is not difficult. Discuss with us if you use an eventing platform ideally capable of streaming - ensuring compliance with e2e-testing is of importance to us.
